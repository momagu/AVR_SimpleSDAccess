...
#define read32(p) (((uint32_t)(p)[3])<<24 | ((uint32_t)(p)[2]) << 16 | (p)[1] << 8 | (p)[0] )
//#define write16(p, i) { *((uint16_t*) (p)) = i; }
#define write16(p, i) { (p)[1] = i>>8; (p)[0] = i & 0xff;}
//#define write32(p, i) { *((uint32_t*) (p)) = i; }
#define write32(p, i) { (p)[1] = (i>>24) & 0xff; (p)[2] = (i>>16) & 0xff; (p)[1] = (i>>8) & 0xff; (p)[0] = i & 0xff;}
#else
uint16_t read16(const uint8_t* p);
uint32_t read32(const uint8_t* p);
void write16(uint8_t* p, uint16_t i);
